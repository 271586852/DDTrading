...
由 Polars / DuckDB 等加载日线并转为 ``List[DailyData]`` 后，调用
``analyze_stock_from_klines``；不包含外部技能仓库或 SQLite 读库逻辑。
"""
...
def bar_dict_rows_from_daily_data(klines: List[DailyData]) -> List[Dict[str, Any]]:
    """战法 / 选股用的字典 K 线（含 ``is_rise`` / ``is_beidou`` 等派生字段）。"""
...
def calculate_sell_score(klines: List[DailyData]) -> Tuple[int, str, Dict[str, bool]]:
    ...
    desc = "；".join(reasons) if reasons else "满分"
...
def analyze_stock_from_klines(ts_code: str, klines: List[DailyData]) -> IndicatorResult:
    """
    综合分析单只股票（由调用方提供已排序的 K 线序列）。

    Args:
        ts_code: 股票代码（展示用，可与 ``DailyData.ts_code`` 一致）
        klines: 按交易日期升序的日线
    ...
    # 卖出评分
    sell_score, _sell_desc, sell_items = calculate_sell_score(klines)
...
def format_result(
    ...
    *,
    ...
    若传入 ``klines``（与本次分析同源、建议不少于 120 根），则附带砖型图文本可视化。
    """
    lines = [
        f"{'='*60}",
...
# ==================== Z 哥战法（字典 K 线） ====================
# 与上方 `DailyData` / `calculate_ma` 等并存；本段仅消费 `List[Dict]` 形态的 bar。
...
def _warfare_kdj_dict(klines: List[Dict], period: int = 9) -> Tuple[float, float, float]:
    """计算 KDJ 指标"""
    if len(klines) < period:
        return 50, 50, 50
...
def _warfare_bbi_dict(klines: List[Dict]) -> float:
    """计算 BBI"""
    if len(klines) < 24:
        return 0
    closes = [k['close'] for k in klines]
...
def detect_b1(klines: List[Dict], index: int) -> Optional[StrategySignal]:
    ...
    today = klines[index]
    k, d, j = _warfare_kdj_dict(klines[:index+1])
    ...
    # B1 买点
    bbi = _warfare_bbi_dict(klines[:index+1])
    price = today['close']
...
def detect_b2(klines: List[Dict], index: int) -> Optional[StrategySignal]:
    ...
    for i in range(5, min(15, index)):
        pk, pd, pj = _warfare_kdj_dict(klines[:index-i+1])
        prev_j_list.append(pj)
        if pj < -10:
            has_b1 = True
            break
...
def detect_changan(klines: List[Dict], index: int) -> Optional[StrategySignal]:
    ...
    # 第一天：B1（J<-13）
    k1, d1, j1 = _warfare_kdj_dict(klines[:index-1])
    if j1 >= -13:
        return None

    # 第二天：放量长阳，J拐头
    k2, d2, j2 = _warfare_kdj_dict(klines[:index])
...
__all__ = [
    ...
    "IndicatorResult",
    "TradeSignal",
    "analyze_stock_from_klines",
    "format_result",
    "bar_dict_rows_from_daily_data",
    "StrategyType",
    "StrategySignal",
    "detect_b1",
    "detect_all_strategies_from_klines",
    "get_latest_signal_from_klines",
    "analyze_with_strategies_from_klines",
    "format_signal",
]
...
def main() -> None:
    """原 SQLite CLI 已移除；请在宿主应用中加载 K 线后调用 ``analyze_stock_from_klines``。"""
    raise SystemExit(
        "请先将日线转为 List[DailyData]，再调用 analyze_stock_from_klines(ts_code, klines)。"
    )
